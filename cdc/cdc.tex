...
\paragraph{Les vaisseaux sanguins}
Chaque vaisseau sanguin est constitué d'un fichier de type \emph{cfg}. Ce fichier doit porter l'extension \textbf{vessel} et être situé dans le dossier \emph{organs/vessels}. Il doit indiquer le chemin vers l'image à utiliser (image de même taille que le corp, avec fond transparent), l'organe d'origine du vaisseau ainsi que l'organe de destination et enfin la longueur du vaisseau en centimètres. Par exemple, un fichier pourrait être :
\begin{verbatim}
